...
public class MD5Encrypt {
    ...
    private String E(String inbuf, String charset)
            throws UnsupportedEncodingException {
        byte[] bytesBuf;
        if (charset == null) {
            bytesBuf = inbuf.getBytes();
        } else {
            bytesBuf = inbuf.getBytes(charset);
        }
        md5Init();
        md5Update(bytesBuf, inbuf.length());
        md5Final();
    ...
    public String E32(String inbuf) {
        try {
            return E32(inbuf, null);
    ...
    public String E32(String inbuf, String charset)
            throws UnsupportedEncodingException {
        return E(inbuf, charset);
    }

    public String E16(String inbuf) {
        try {
            return E16(inbuf, null);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public String E16(String inbuf, String charset)
            throws UnsupportedEncodingException {
        return E(inbuf, charset).substring(8, 24).toLowerCase();
    }
